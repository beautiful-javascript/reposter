...
function fetcher() {
  return {
    fetch: function fetch(sources, eventBus) {
      return _bluebird2.default.reduce(sources, function (partialResult, source) {
        var fetchStrategy = source.fetchStrategy(source.fetchStrategyOptions, eventBus, source.id);
        var parser = source.parser(source.parserOptions, eventBus, source.id);
